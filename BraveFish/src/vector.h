...
template <>
Vector<2, float> Vector<2, float>::Zero = Vector<2, float>(0, 0);
template <>
Vector<2, float> Vector<2, float>::One = Vector<2, float>(1, 1);
...
template <size_t SIZE, typename T>
T operator*(Vector<SIZE, T> lhs, const Vector<SIZE, T>& rhs) {
    T ret = T();
...
Vector<SIZE, T> operator*(Vector<SIZE, T> lhs, const U& f) {
...
template <size_t TARGET_SIZE, size_t SIZE, typename T>
Vector<TARGET_SIZE, T> embed(const Vector<SIZE, T>& v, T fill = 1) {
...
template <size_t TARGET_SIZE, size_t SIZE, typename T>
Vector<TARGET_SIZE, T> proj(const Vector<SIZE, T>& v) {
    Vector<TARGET_SIZE, T> ret;
...
template <size_t SIZE, typename T>
std::ostream& operator<<(std::ostream& out, Vector<SIZE, T>& v) {
