...
Vector3f Model::vert(int i) { return verts_[i]; }

Vector3f Model::vert(int iface, int nthvert) { return verts_[faces_[iface][nthvert][0]]; }
...
void Model::load_texture(std::string filename, const char* suffix, TGAImage& img) {
    ...
    if (dot != std::string::npos) {
        ...
        // std::cerr << "texture file " << texfile << " loading " << (img.read_tga_file(texfile.c_str()) ? "ok" :
        // "failed") << std::endl;
...
Vector2f Model::uv(int iface, int nthvert) { return uv_[faces_[iface][nthvert][1]]; }
...
TGAImage* Model::get_diffuse_map() { return &diffusemap_; }

TGAImage* Model::get_normal_map() { return &normalmap_; }
TGAImage* Model::get_specular_map() { return &specularmap_; }
